...
exports.customerSignup = async (request, response, next) => {
    try {
        ...
        if (!customerExist) {
            ...
            const customer = await Customer.create({ name, email, phonenumber, password: hash });
            await customer.createCart();
            const token = jwt.sign({ customerId: customer.id }, secretKey, { expiresIn: '1h' });
...
exports.CustomerSignin = async (request, response, next) => {
    try {
        ...
        let customerExist = await Customer.findOne({ where: { email } })
        if (customerExist) {
            const isPasswordValid = await bcrypt.compare(password, customerExist.password);
            if (isPasswordValid) {
                const token = jwt.sign({ customerId: customerExist.id }, secretKey, { expiresIn: '1h' });
                response.cookie('token', token, { maxAge: 3600000 });
